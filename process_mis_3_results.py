...
# Read the JSON file
with open('results_mis_3_prod_multidimensional_aux.txt') as f:
    data = json.load(f)
...
for test in data.values():
    for heuristic, sample_data in test.items():
        for sample_size, stats in sample_data.items():
            variances[heuristic][sample_size].append(stats['mean of variance'])
            errors[heuristic][sample_size].append(stats['mean of error'])
            std_devs[heuristic][sample_size].append(stats['mean of standard deviation'])
...
for i, test in enumerate(data.values()):
    ...
    for heuristic, sample_data in test.items():
        for sample_size, stats in sample_data.items():
            variances[heuristic].append(stats['mean of variance'])
            errors[heuristic].append(stats['mean of error'])
            std_devs[heuristic].append(stats['mean of standard deviation'])
    ...
    for heuristic, sample_data in test.items():
        for sample_size, stats in sample_data.items():
            if heuristic == 'balance':
                balance_variances.append(stats['mean of variance'])
                balance_errors.append(stats['mean of error'])
                balance_std_devs.append(stats['mean of standard deviation'])
            elif heuristic == 'power':
                power_variances.append(stats['mean of variance'])
                power_errors.append(stats['mean of error'])
                power_std_devs.append(stats['mean of standard deviation'])
            elif heuristic == 'maximum':
                maximum_variances.append(stats['mean of variance'])
                maximum_errors.append(stats['mean of error'])
                maximum_std_devs.append(stats['mean of standard deviation'])
            elif heuristic == 'cutoff':
                cutoff_variances.append(stats['mean of variance'])
                cutoff_errors.append(stats['mean of error'])
                cutoff_std_devs.append(stats['mean of standard deviation'])
            elif heuristic == 'sbert':
                sbert_variances.append(stats['mean of variance'])
                sbert_errors.append(stats['mean of error'])
                sbert_std_devs.append(stats['mean of standard deviation'])
